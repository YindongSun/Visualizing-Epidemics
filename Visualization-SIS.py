...
# SIS - Simulate Disease Spread
def SIS (G, r0, ti, k, days):
    ...
    s_temp=[]
    s_temp.append(initial_adopters)
    t = 0
    e = [[]]
    while t<days:
        if len(s[t])==0 or len(i[t])==0:
            break
        new_adopters = []
        new_edges = []
        for node in i[t]:
        ...
        s_temp.append(new_adopters)
        t += 1
        if t>= ti:
            i[t] = list(set(i[t]) - set(s_temp[t - ti]))
            s[t] = s[t] + s_temp[t - ti]
    return t, s, i, e
...
def update(frame):
    ...
    ax.set_title("SIS Model" + " Day " + str(frame), fontweight="bold")
    ax.set_xticks([])
    ax.set_yticks([])
